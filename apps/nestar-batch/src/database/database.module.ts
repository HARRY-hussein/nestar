...
@Module({
	// NestJS da — Databasega ulanish uchun mas'ul modul.
	imports: [
		// Boshqa modullardan kelgan narsalarni shu modulga kiritish uchun.
		MongooseModule.forRootAsync({
			// database ulanishidan oldin biror narsa kutish kerak bo'lsa. Bu yerda .env faylidan o'zgaruvchilarni o'qishi va ular tayyor bo'lguncha kutish kerak
			useFactory: () => ({
				// OPTION: Sozlamalarni funksiya orqali qaytaradi. Oddiy object yozish o'rniga funksiya ishlatiladi
...
export class DatabaseModule {
	constructor(@InjectConnection() private readonly connection: Connection) {
		// "NestJS, menga MongoDB ulanish obyektini ber": avtomatik instance olib beradi: const connection = new Connection()
		// private — faqat shu class ichida ishlatiladi; readonly — bir marta berilgandan keyin o'zgartirib bo'lmaydi.
		if (connection.readyState === 1) {
			// Ulanish holati tekshiriladi: 1 — ulangan
			console.log(
